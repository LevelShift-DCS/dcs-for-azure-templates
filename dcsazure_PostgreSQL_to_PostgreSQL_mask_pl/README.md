...
This pipeline will perform masking of your PostgreSQL DB (NoSQL API) data.
...
1. [Create an Azure Function app for exporting PostgreSQL DB data to Azure Data Lake Storage (ADLS)](https://help.delphix.com/dcs/current/content/docs/create_an_azure_function.htm) (version `Cosmos_to_ADLS_V1`).
...
1. [Configure an Azure Key Vault for storing the PostgreSQL DB access key and assign a managed identity with the Key Vault Secrets User role to the Azure Function](https://help.delphix.com/dcs/current/content/docs/configure_azure_function_access_to_cosmos_db_secret_using_azure_key_vault.htm).
...
`Azure Function` (Cosmos to ADLS) – Linked service associated with exporting PostgreSQL DB data to ADLS. This will be used for the following steps:
...
* Check If We Should Copy Data To Cosmos
  * Copy ADLS Data to Cosmos
    * Export documents from ADLS to PostgreSQL DB container using an Azure Function
...
* `DATASET` – Dataset identifier used in the metadata store (default `POSTGRESQL`)
* `METADATA_EVENT_PROCEDURE_NAME` – Stored procedure used to capture masking execution events and update masking state (default `insert_adf_masking_event`)
* `METADATA_MASKING_PARAMS_PROCEDURE_NAME` – Stored procedure used to generate PostgreSQL masking parameters (default `generate_cosmos_no_sql_masking_parameters`)
* `COLUMN_WIDTH_ESTIMATE` – Estimated column width used for batch size calculation when schema width is unavailable (default `1000`)
* `STORAGE_ACCOUNT` – Azure Data Lake Storage account name used for staging masked data
* `COSMOS_TO_ADLS_BATCH_SIZE` - This is the number of rows per batch while copying the data from PostgreSQL database to ADLS (default `50000`)
* `COSMOS_KEY_VAULT_NAME` – Name of the Azure Key Vault that stores the PostgreSQL DB access key
* `COSMOS_SECRET_NAME` – Name of the secret in Key Vault containing the PostgreSQL DB access key

### Parameters

* `P_COSMOS_SOURCE_DATABASE` – String – Source PostgreSQL DB database name
* `P_COSMOS_SINK_DATABASE` – String – Target PostgreSQL DB database name for masked data
* `P_COSMOS_SINK_ENDPOINT` – String – PostgreSQL DB endpoint URL for the masked target
* `P_COSMOS_SINK_KEY` – SecureString – PostgreSQL DB access key for the masked target
* `P_COSMOS_CONTAINER` – String – PostgreSQL DB container name
...
* `P_REAPPLY_MAPPING` – Bool – Reapply source-to-sink mapping before masking (default `true`)
* `P_COPY_ADLS_DATA_TO_COSMOS` – Bool – Specifies whether data should be copied from ADLS to PostgreSQL DB (default `true`)
...
* Update the `COSMOS_KEY_VAULT_NAME` and `COSMOS_SECRET_NAME` variables to match the target PostgreSQL DB account before triggering the pipeline.
...
* Ensure that all schemas associated with the PostgreSQL DB container are added to the `adf_data_mapping` table before triggering the masking pipeline.
