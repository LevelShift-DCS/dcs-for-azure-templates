...
- Spanner_to_ADLS ZIP file containing the Azure Function App code and configuration
...
1. Zip the Spanner_to_ADLS Azure Functions app code and configuration into a single archive file (e.g., `Spanner_to_ADLS.zip`).
   ```
   zip Spanner_to_ADLS.zip host.json function_app.py requirements.txt
   ```
...
* `resource-group` : The Azure resource group that contains the Spanner_to_ADLS Azure Function App
* `name` : The name of the Spanner_to_ADLS Azure Function App
* `src` : The full path to the Spanner_to_ADLS ZIP package containing the Function App code
